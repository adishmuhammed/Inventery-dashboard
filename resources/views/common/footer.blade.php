<!-- start site-footer -->
<footer class="site-footer">
    <div class="upper-footer">
        <div class="container">
            <div class="row">
                <div class="col col-md-4 col-sm-6">
                    <div class="widget about-widget">
                        <div class="footer-logo icon-wrapper"><img src="{{ asset('assets/images/logo.webp') }}" alt>
                            <span class="logo-title">
                                <h4>Sahayi</h4>
                                <h4>Dialysis</h4>
                                <h4>Center</h4>
                            </span>
                        </div>
                        <p>Sahayi Dialysis Center is a diaplsy center in medical collage helpping people to get better
                            health</p>
                        <ul class="contact-info">
                            <li><i class="fa fa-phone"></i> 096567 11325 </li>
                            <li><i class="fa fa-envelope"></i> info@sahayidialysis.com</li>
                            <li><i class="fa fa-home"></i> Madathumpoyil Road, Poonoor, Unnikulam P.O, Calicut, Kerala
                                673574
                            </li>
                        </ul>
                    </div>
                </div>

                <div class="col col-md-4 col-sm-12">
                    <div class="widget links-widget">
                        <h3>Links</h3>
                        <div class="row">
                            <div class="col-md-6 ">
                                <ul class="margin-right">
                                    <li><a href="{{ url('/') }}">Home</a></li>
                                    <li><a href="{{ url('services.html') }}">Dialysis</a></li>
                                    <li><a href="{{ url('service-single-finance.html') }}">Diploma In Dialysis
                                            Technology (DDT)</a></li>
                                </ul>
                            </div>
                            <div class="col-md-6 ">
                                <ul class="margin-right">
                                    <li><a href="{{ url('/blogs') }}">Blogs</a></li>
                                    <li><a href="{{ url('/contact-us') }}">Contact</a></li>
                                    <li><a href="{{ url('/gallery') }}">Gallery</a></li>
                                    <li><a href="{{ url('/about-us') }}">About</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col col-md-3 col-sm-6">
                    <div class="widget twitter-feed-widget">
                        <h3>Testimonials</h3>
                        <ul>
                            <li>
                                <div class="text">
                                    <p>The NGO services giving free treatment to the poor kidney failure patients is
                                        much appreciated</p>
                                </div>
                                <div class="info-box">
                                    &nbsp;➖&nbsp;
                                    <strong><a href="{{ url('#') }}">Rashid koduvally (Director,
                                            Pensare)</a></strong>
                                </div>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div> <!-- end upper-footer -->
    <div class="copyright-info">
        <div class="container">
            <div class="row">
                <div class="col col-xs-6">
                    <div class="copyright-area">
                        <p>2023 &copy; Developed by <a href="http://pensare.in/">pensare marketing services</a></p>
                    </div>
                </div>
                <div class="col col-xs-6">
                    <div class="footer-social">
                        <span>Follow us: </span>
                        <ul class="social-links">
                            <li><a href="{{ url('#') }}"><i class="fa fa-facebook-square"></i></a></li>
                            <li><a href="{{ url('#') }}"><i class="fa fa-twitter-square"></i></a></li>
                            <li><a href="{{ url('#') }}"><i class="fa fa-linkedin-square"></i></a></li>
                            <li><a href="{{ url('#') }}"><i class="fa fa-google-plus-square"></i></a>
                            </li>
                            <li><a href="{{ url('#') }}"><i class="fa fa-rss-square"></i></a></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
</footer>
